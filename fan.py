...
# [핵심] 소음 데이터에서 dB와 dB(A)를 모두 추출하는 함수
def get_noise_pair(model_data, keyword):
    for col in model_data.index:
        if keyword.lower() in col.lower():
            ...
            if '/' in raw_val:
                parts = raw_val.split('/')
                db_val = parts[0].strip().replace('dB', '').strip()
                dba_val = parts[1].strip().replace('dB(A)', '').strip()
                return f"{db_val} / {dba_val}"
...
def create_noise_chart(model_data):
    ...
    vals = [float(get_noise_pair(model_data, b).split('/')[0]) for b in bands]
...
def create_final_pdf(p_info, model_data, chart_buf, noise_buf, d_point):
    ...
    # Project Info & Design Info (V7.9 레이아웃)
    ...
    # [수정] 소음 표 생성 (dB / dB(A) 병행 표기)
    table_y = h-520
    p.setLineWidth(0.5); p.setFillColor(colors.lightgrey); p.rect(50, table_y, 495, 22, fill=1)
    p.setFillColor(colors.black); p.setFont("Helvetica-Bold", 8) # 폰트 살짝 조정
    ...
    for i, kw in enumerate(keywords):
        ...
        p.drawCentredString(77 + (i*55), table_y + 7, f"{pair}")
        p.line(50 + (i*55), table_y, 50 + (i*55), table_y + 22)
    p.line(545, table_y, 545, table_y + 22)
    
    p.setFont("Helvetica-Oblique", 8); p.drawString(50, table_y - 15, "* Note: Noise levels are shown as 'dB / dB(A)'")
