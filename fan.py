...
# 페이지 설정
st.set_page_config(page_title="루트에어 선정 시스템 V7.9.2", layout="wide")
...
# [핵심 수정] 소음 데이터에서 dB와 dB(A)를 추출하여 반환하는 함수
def get_noise_display_val(model_data, keyword):
    for col in model_data.index:
        if keyword.lower() in col.lower():
            val = str(model_data[col]).strip()
            # '89 / 91' 형태인 경우 그대로 반환, 단위가 붙어있으면 정리
            val = val.replace('dB(A)', '').replace('dB', '').strip()
            if '/' in val:
                return val
            # 데이터에 값이 하나만 있는 경우 (예: '89') 임의의 계산값이 아닌 원본 유지
            return f"{val} / {val}" 
    return "0 / 0"

# 2. 메인 성능 맵 생성 (V7.9 데이터 기반 서징 로직)
...
def create_noise_chart(model_data):
    ...
    vals = [float(get_noise_display_val(model_data, b).split('/')[0]) for b in bands]
...
def create_final_pdf(p_info, model_data, chart_buf, noise_buf, d_point):
    ...
    for i, kw in enumerate(keywords):
        pair_val = get_noise_display_val(model_data, kw)
        p.drawCentredString(77 + (i*55), table_y + 7, pair_val)
        p.line(50 + (i*55), table_y, 50 + (i*55), table_y + 22)
    p.line(545, table_y, 545, table_y + 22)
    
    p.setFont("Helvetica-Oblique", 8); p.drawString(50, table_y - 15, "* Noise Values are presented as 'dB / dB(A)'")
...
if df is not None:
    ...
    with header_col2:
        st.markdown("<h1 style='margin-top: 25px;'>루트에어 송풍기 선정 시스템 V7.9.2</h1>", unsafe_allow_html=True)
    ...
    pdf_file = create_final_pdf(p_info, model_data, create_master_chart(df, selected_model, u_cmh, u_pa), create_noise_chart(model_data), d_point)
